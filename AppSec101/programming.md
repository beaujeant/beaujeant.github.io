...
> __Note__: More types are available, but those are the most used.
...
It automatically converted the float `3.2` into the integer (`0x3`) and copied it in memory where the variable `number` is located (i.e. `ebp-0xc`). Don't worry if you didn't understand this last example, we will have a closer look at it in the next [chapter](/appsec101/assemlby/).
...
The above assembly instruction allocates 4 bytes in the stack. `ESP` points at the top of the stack, and if you look in chapter [memory](/appsec101/memory/), the stack is growing _backward_, so subtracting `ESP` means increasing the size of the stack.
...
> __Note__: Single quotes (`'`) is interpreted in C as a single character, while double quotes (`"`) are interpreted as a `NULL` terminated string.
...
> __Note__: The specifier `%n` expect a memory address (pointer) and not a variable, hence the `&i`.
...
> __Note__: The prototype of the function `strcpy` is located in the _header file_ `string.h`, so you need to `#include` it first at the beginning of your code.
...
```
Once again, for more information, you can read the [free reference](http://www.cplusplus.com/reference/cstdlib/free/). We will see more about this function in the chapter [use-after-free](#).

That's it for this big chapter about C programming. In the [next chapter](/appsec101/assembly/), we will see how this is translated by the compiler in assembly.
